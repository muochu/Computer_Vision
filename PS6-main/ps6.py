...
class Boosting:
    ...
    def train(self):
        ...
        for j in range(self.num_iterations):
            ...
            h.train()
            h_predictions = np.array([h.predict(x) for x in self.Xtrain])
            epsilon_j = np.sum(self.weights[h_predictions != self.ytrain])
            
            if epsilon_j > self.eps:
                alpha_j = 0.5 * np.log((1 - epsilon_j) / epsilon_j)
                self.weights = self.weights * np.exp(-self.ytrain * alpha_j * h_predictions)
    ...
    def predict(self, X):
        ...
        for x in X:
            weighted_sum = 0.0
            for alpha, weak_clf in zip(self.alphas, self.weakClassifiers):
                weighted_sum += alpha * weak_clf.predict(x)
...
class HaarFeature:
    ...
    Attributes:
        ...
        size (tuple): Feature's height and width.
    """
...
class CascadeClassifier:
    ...
    def evaluate_classifiers(self, pos, neg, classifiers):
        ...
        Args:
            pos (list): List of positive images.
            neg (list): List of negative images.
            classifiers (list): list of ViolaJones classifiers to predict where index i is the i'th 
                consecutive ViolaJones classifier
